...
function Moviecard({ movie }) {
    ...
    const [userRating, setUserRating] = useState(0); // Initialize the user's rating
    const maxRating = 5; // Maximum rating value
  
    // Define a function to handle changes in the rating
    const handleRatingChange = (newRating) => {
      setUserRating(newRating); // Update the user's rating when a star is clicked
      const requestData = {
        newRating: newRating, // Replace with the desired new rating
      };
      axios.post(`http://localhost:8000/modify-rating?id=${movie.id}`, requestData)
        .then((response) => {
    ...
    return (
        <motion.div
            ...
            {/* bookmark buttons */}
            <button className="absolute bg-black text-white p-2 z-20 right-0 m-3 rounded-full text-xl"> <MovieRating initialRating={movie.rating} onRate={handleRatingChange} /></button>
